        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              {activeChatId === 'global' ? `${users.length + 1} members` : (users.find(u => u.id === activeChatId)?.lastOnline || 'Offline')}
            </p>
          </div>
        </div>
      </div>
            ...
            >
              <div className="flex items-end gap-2 max-w-[80%]">
                {!isMe && (
                  <img 
                    src={sender.avatar} 
                    className="w-8 h-8 rounded-lg object-cover mb-1 cursor-pointer" 
                    alt="" 
                ...
                <div className="relative">
                  ...
                  <div className={`
                    ...
                    ${isMe ? 'bg-indigo-600 text-white rounded-tr-none shadow-lg shadow-indigo-600/20' : 'bg-slate-800 text-slate-200 rounded-tl-none border border-slate-700'}
                  `}>
              ...
              {longPressMessage === msg.id && (
                <div className={`
                  absolute z-20 bg-slate-800 border border-slate-700 p-2 rounded-2xl shadow-2xl flex gap-2 animate-in fade-in zoom-in duration-200
                  ${isMe ? '-translate-x-full' : 'translate-x-12'}
                `}>
                  {EMOJIS.map(e => (
                    <button 
                      key={e}
      ...
      <div className="p-6 bg-slate-900 border-t border-slate-800">
        <form onSubmit={handleSend} className="relative flex items-center gap-3">
          <input 
            ...
            className="flex-1 bg-slate-800 border border-slate-700 rounded-2xl px-6 py-4 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all placeholder-slate-500 pr-16"
            placeholder="Type your message..."
          />
          <button 
            type="submit"
            disabled={!inputText.trim()}
            className="absolute right-2 p-3 bg-indigo-600 text-white rounded-xl hover:bg-indigo-500 transition-all active:scale-90 disabled:opacity-50 disabled:scale-100"
          >
